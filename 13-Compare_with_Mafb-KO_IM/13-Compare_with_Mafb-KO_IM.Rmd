...
```{r fig.height=4, fig.width=5}
VlnPlot(so, features = "sig.IM", cols = pal4, pt.size = 0)
```
```{r}
dt <- data.frame(sig=so$sig.IM, celltype=so$cell.type2)
...
```{r fig.height=4, fig.width=5}
VlnPlot(so, features = "sig.pMo", cols = pal4, pt.size = 0)
```
```{r}
...
```{r}
dt <- data.frame(sig=so$sig.cMo, celltype=so$cell.type2)
res.aov <- aov(sig ~ celltype, data = dt)
summary(glht(res.aov, linfct = mcp(celltype = "Tukey")))
```

```{r fig.height=4, fig.width=5}
VlnPlot(so, features = "sig.cMo", cols = pal4, pt.size = 0)
```
```{r}
