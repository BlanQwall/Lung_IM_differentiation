...
In this experiment, we have 4 groups of chimera mice (host = CD45.1-IM-DTR): 
    ...
    1. Three chimera with littermate control BM (control group); 
    2. Three chimera with Ms4a3-cMAF BM  (cMAF-KO group);
    3. Three chimera with Ms4a3-MAFb BM  (MAFb-KO group);
    4. Three chimera with Ms4a3-cMAF/MAFb BM  (cMAF/MAFb-dKO group). 
...
```{r fig.width=16, fig.height=5}
results$cell.type2 <- factor(Idents(results), labels = c("MHCII IM", "CD206 IM", "Hi-Mt", "Patrolling Mono", "Classical Mono", "Unknown"))
Idents(results) <- "cell.type2"
DimPlot(results, label = TRUE, split.by = "group", dims = c(2,3))
```
...
# try to output the cell barcodes for BAM subset

```{r eval=FALSE}
subset_by <- "group"
so <- results

for (x in unique(so[[subset_by]][,1])) {
  meta.data <- so[[]]
  cellnames <- rownames(meta.data[meta.data[[subset_by]] == x, ])
  write.table(cellnames, file = paste("./cellnames_" , x, "_by_", subset_by,".cellnameSubset.csv", sep = ""), row.names = FALSE, col.names = FALSE, quote = FALSE)
}

```

Run the following bash file for BAM subset and index. 
```{bash eval=FALSE}
# for BAM subset
bash 20220112_subset_BAM.sh | tee -a ../logs/20220112_subset_BAM
# for index BAM
bash 20220112_SAM_index.sh | tee -a ../logs/20220112_SAM_index
```


```{r}
freq.celltype.list <- list(
  `Control` = Seurat2CellFreqTable(subset(results, subset = group == "HT5-Control"), slotName = "Phase"), 
  `cMAF-KO` = Seurat2CellFreqTable(subset(results, subset = group == "HT6-cMAF-KO"), slotName = "Phase"), 
  `MAFb-KO` = Seurat2CellFreqTable(subset(results, subset = group == "HT7-MAFb-KO"), slotName = "Phase"), 
  `dKO` = Seurat2CellFreqTable(subset(results, subset = group == "HT8-dKO"), slotName = "Phase") 
)
...
# apoptosis
```{r fig.height=5, fig.width=12}
FeaturePlot(results, features = "percent.mt", reduction = "umap", split.by = "group", dims = c(2,3))

```

```{r}
saveRDS(results, file = "./cellType_filtered.withSingleR.seuratObject.Rds")
