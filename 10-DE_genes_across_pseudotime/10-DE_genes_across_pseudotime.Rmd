...
Give another gene set with lower threshold: 
```{r}
genes.2 <- row.names(subset(modulated_genes, q_value < 0.02 & morans_I > 0.2))
length(genes.2)
```
*Probably not do this because the median of morans_I is alread 0.025, very low.*
...
# library(seriation)
#o1 <- seriate(dist(pt.matrix), method = "GW")
# o2 <- order(modulated_genes[rownames(pt.matrix), "morans_test_statistic"])
hthc <- Heatmap(
  pt.matrix,
  name                         = "z-score",
  col                          = colorRamp2(seq(from=-2,to=2,length=11),rev(brewer.pal(11, "Spectral"))),
